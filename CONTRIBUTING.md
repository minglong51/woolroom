...
link, and the loader runs it from any local directory. A linked-list index
of community packs opens at launch; until then, open an issue with the
**pack submission** template: your repo link plus the output of
`scripts/pack_lint.py <pack-dir> --strict` run against a checkout of this
repo. The authoring loop is documented in [docs/packs.md](docs/packs.md).
