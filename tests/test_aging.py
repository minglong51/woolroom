...
def test_life_stage_thresholds() -> None:
    assert life_stage(_ago(0)) == "juvenile"
    assert life_stage(_ago(60)) == "juvenile"   # ~2 pet years
...
def test_render_scale_per_stage() -> None:
    assert render_scale("juvenile") < render_scale("young") < render_scale("adult")
    assert render_scale("senior") < render_scale("adult")


def test_aging_handles_missing_adopted_at() -> None:
    assert pet_age_years(None) == 0.0
    assert life_stage(None) == "juvenile"


def test_stage_proportions_capture_silhouette_intent() -> None:
    juvenile = stage_proportions("juvenile")
    adult = stage_proportions("adult")
    senior = stage_proportions("senior")

    # Juvenile has a relatively bigger head than the smaller body.
    assert juvenile["headScale"] > adult["headScale"]
    assert juvenile["bodyScaleX"] < adult["bodyScaleX"]
    assert juvenile["tailScale"] < adult["tailScale"]
