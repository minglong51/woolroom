...
def _load_manifest(
    root: Path, budget: _Budget, environment: PackEnvironment
) -> dict[str, Any]:
    rel = Path("pack.yaml")
    ...
    if fx > environment.fx_vocab_version:
        raise PackVocabError(
            f"pack {data['name']!r} was built against fx vocabulary v{fx}; "
            f"this engine speaks v{environment.fx_vocab_version}"
        )
    return data
...
def _load_phrases(
    root: Path,
    budget: _Budget,
    pack_species: dict[str, dict[str, Any]],
    environment: PackEnvironment,
    ...
    for path in _iter_subdir(root, "phrases", ".yaml"):
        ...
        rel = path.relative_to(root)
        if species_id not in pack_species and species_id not in environment.species_ids:
            raise PackPhraseError(
        ...
        for table, known_keys in (
            ("action", environment.action_ids),
            ("spot", environment.spot_ids),
        ):
...
def _validate_when(
    ...
    unknown = set(_condition_keys(when)) - environment.condition_ids
    if unknown:
...
def _validate_scene_fx(
    ...
    if mode not in environment.fx_modes:
...
def _validate_quirk_rule(
    ...
    _validate_when(quirk_id, channel, rule, environment)
    if channel == "pose":
        ...
        rig_keys = environment.pose_keys
        for key, spec in write.items():
            ...
            if isinstance(spec, dict):
                ...
                unknown_ops = set(spec) - environment.pose_write_ops
                if unknown_ops:
                    raise PackQuirkError(
    ...
    if channel == "events":
        ...
        if emit["type"] not in environment.quirk_emit_types:
            raise PackQuirkError(
                f"{where} emit type {emit['type']!r} is not in the emit vocabulary "
                f"(QUIRK_EMIT_TYPES: {sorted(environment.quirk_emit_types)})"
            )
        if not isinstance(emit.get("data"), dict):
            raise PackQuirkError(f"{where} emit 'data' must be a mapping")
    if "scene_fx" in rule:
        _validate_scene_fx(quirk_id, channel, rule, environment)
...
def _load_quirks(
    ...
    for path in _iter_subdir(root, "quirks", ".yaml"):
        ...
        for channel, rules in behavior.items():
            ...
            for rule in rules:
                _validate_quirk_rule(quirk_id, channel, rule, environment)
        defs[quirk_id] = data
    return defs
...
def validate_pack(
    path: str | Path,
    *,
    environment: PackEnvironment = DEFAULT_ENVIRONMENT,
) -> ValidatedPack:
    root = _confine_root(path)
    budget = _Budget()
    manifest = _load_manifest(root, budget, environment)
    species, raw_svgs = _load_species(root, budget)
    overlays = _load_phrases(root, budget, species, environment)
    quirks = _load_quirks(root, budget, environment)
    voice = _load_voice(root, budget)

    for species_id in species:
        if species_id in environment.species_ids:
            raise PackCollisionError(
                f"species {species_id!r} collides with an already-registered species"
            )
    for species_id in overlays:
        if species_id in environment.overlay_ids:
            raise PackCollisionError(f"phrase overlay for {species_id!r} already exists")
    for quirk_id in quirks:
        if quirk_id in environment.quirk_ids:
            raise PackCollisionError(
    ...
    record = PackRecord(
        ...
        phrase_languages={sid: overlay["language"] for sid, overlay in overlays.items()},
    )
